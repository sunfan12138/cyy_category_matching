...
- **示例**：茶百道 → 奶茶、果茶、新式茶饮、现制饮品；蜜雪冰城 → 奶茶、冰淇淋、茶饮、现制饮品。目的是让规则库里的关键词能命中你输出的这些词。
- 只输出一句，不要多句、不要分点、不要换行；不要解释原因、不要加序号或标题。
- 尽量包含多个相关品类词（同义、近义或上下游业态），便于匹配命中。
...
**调用方式**：发起一次工具调用，把用户原文或需查的内容作为查询参数传入；收到返回后，从返回内容中提炼品类词，按「多个词用顿号/逗号连接」的格式输出，尽量使用上文给出的参考词汇。若搜索仍无结果，直接返回「未匹配到结果」。
"""
