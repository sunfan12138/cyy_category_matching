...
async def get_category_description_with_search_async(
    ...
    """
    异步调用大模型生成品类描述，可使用 MCP 工具（如搜索）；无工具或调用失败时返回 None。
    供 asyncio.gather 等批量并发使用。
    """
    ...
    try:
        out = await _run_llm_with_tools(
            category_text, rules, api_key, base_url, model, config_list, session_events, _flush_log
        )
...
async def _run_llm_with_tools(
    ...
    """内部：在 MCP 管理器中跑多轮对话（AsyncOpenAI + 工具调用）。"""
...
def get_category_description_with_search(
    ...
    同步入口，内部使用 run_async(get_category_description_with_search_async(...))。
    """
    try:
        from mcp_client import run_async
    except ImportError:
        return None
    return run_async(get_category_description_with_search_async(category_text, rules))
